...
public class AutoGapple extends ModuleWithModuleSettings {
    public AutoGapple() {
        ...
        this.registerSetting(new NumberSetting("Health", "Maximum health before eating gapple.", 7.0F, 0.5F, 10.0F, 0.5F));
    }
            ...
            if (mc.player.container.getSlot(slotIndex).getHasStack()) {
                ...
                if (stack != null) {
                    if (!checkForFireResistance) {
                        ...
                        List<EffectInstance> effects = InvManagerUtil.getPotionEffects(stack);
                        if (effects != null) {
